...
INSERT INTO inventory (product_id, food_bank_id, quantity, received_date, expiry_date, batch_number)
VALUES
  (
    (SELECT product_id FROM products WHERE product_name = 'Baked Beans'      AND product_brand = 'Heinz'),
    (SELECT id FROM food_banks WHERE name = 'Birmingham Central Food Bank'),
    85, '2025-05-01', '2026-06-01', 'BB001'
  ),
  (
    (SELECT product_id FROM products WHERE product_name = 'Baked Beans'      AND product_brand = 'Heinz'),
    (SELECT id FROM food_banks WHERE name = 'Sparkhill Food Bank'),
  ...
  (
    ...
    (SELECT id FROM food_banks WHERE name = 'Noor E Huda Masjid Food Bank'),
    6, '2025-06-04', '2025-07-04', 'CC001'
  )
ON CONFLICT (product_id, food_bank_id, batch_number) DO NOTHING;
